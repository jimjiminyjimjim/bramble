...
export function SoldOut({
  ...
  const handleFormSubmit = (status) => {
    ...
    if (status === "success") {
      setTimeout(() => {
        document.getElementById("sold_out_modal").close();
        setFormStatus(null);
      }, 2000);
    }
  };
  ...
  const closeModal = () => {
    const modal = document.getElementById("sold_out_modal");
    modal.close();
  };
  ...
  return (
    <>
      {textLink ? (
        <a
          href="#"
          onClick={() => document.getElementById("sold_out_modal").showModal()}
        >
          {ctaText}
        </a>
      ) : (
        <button
          className={`btn`}
          style={{
            backgroundColor: colour ? colour : colors?.button.dark,
            color: "#FFFFFF"
          }}
          onClick={() => document.getElementById("sold_out_modal").showModal()}
        >
      ...
      >
        <div className="modal-box bg-white">
          <h3 className="text-xl font-semibold lg:text-3xl">{ctaPopupTitle}</h3>
          <p className="text-md font-semibold lg:text-md">{ctaPopupBody}</p>
